...
A writing checkpoint declares its ordered bounded turns up front: each turn names one coherent outcome, a narrow surface, explicit non-goals, and an objective targeted check. The scheduler runs those turns serially in one isolated worktree, records each targeted check, then seals one exact checkpoint commit and reruns affected acceptance tests before review. Low-risk checkpoints receive focused review; public-contract, architecture, security/trust-boundary, data-migration, concurrency, expanded-acceptance-scope, or uncertain changes force broad multi-perspective review. After all reviewed checkpoints integrate, an automatic final gate independently reviews and runs every applicable testing layer against the exact combined coordinator commit; only fresh evidence with no unresolved high or critical findings completes the run.
