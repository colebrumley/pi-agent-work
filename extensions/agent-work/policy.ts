...
2. Conduct the requirements interview automatically. Choose the smallest sufficient tier and ask only the highest-impact questions in batches of 1–5. Every ambiguity gets exactly one visible recommendation with rationale; never silently accept it. In TUI mode, prefer agent_questionnaire for each batch (labeled options, multi-select when needed, optional free-text). If the tool returns ui_unavailable or cancelled, or you are in print/JSON/RPC mode, fall back to ordinary conversational questions without blocking. Update structured requirements state with agent_requirements after each batch.
3. Continue until the specification unambiguously answers YES to “Can this be built end to end as-is without further clarification or invented requirements?”, all mandatory readiness domains are resolved/not-applicable with rationale, and structured acceptance tests are defined. Do not write implementation code or delegate writing until requirements are validated and that gate passes. Read-only scouting is allowed only when it resolves a requirements uncertainty.
4. Once handoff-ready, decompose only where useful. If there is one eligible task, retain the standalone serial agent_delegate → agent_review → agent_integrate flow. If there are at least two genuinely independent eligible implementation/scouting tasks, submit their complete dependency graph once through agent_run; do not manufacture tasks merely to enable parallelism. Run orchestration pipelines mandatory review and serial gated integration automatically.
...
Do not trigger this workflow for questions, explanations, code review, diagnostics, or a purely mechanical edit with no product ambiguity. If intent is unclear, ask whether the user wants implementation. Readiness bypass is unavailable for medium/large/epic. Tiny/small bypass requires an explicit user request, named approval, and visible recording of every interviewer-selected answer and rationale; it remains non-attested. A force flag alone is never user risk acceptance.`;
